...
function toItems<T>(input: ListResponse<T> | T[] | null | undefined): T[] {
  if (!input) return [];
...
function useClasses() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<ListResponse<SelectOption>>("/api/v1/classes?pageSize=100");
...
function useTerms() {
  return useQuery({
    queryKey: ["terms-list"],
    queryFn: async () => {
      const res = await api.get<ListResponse<SelectOption>>("/api/v1/terms");
      if (!res.success) return [];
      return toItems(res.data);
    },
  });
...
function useSubjects() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await api.get<ListResponse<SelectOption>>("/api/v1/subjects?pageSize=200");
      if (!res.success) return [];
      return toItems(res.data);
    },
  });
...
export default function TeacherExportPage() {
  ...
  async function doExport(type: ExportType) {
    ...
    try {
      ...
      const filename =
        res.headers.get("Content-Disposition")?.match(/filename="(.+?)"/)?.[1] ?? `${type}_export.${filters.format}`;
    ...
    <div className="page-shell">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div className="card p-6 flex flex-col gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 mt-0.5">Export attendance records with date, status, and class details.</p>
            </div>
          </div>
          <div className="mt-auto">
            <p className="text-xs text-slate-400 mb-3">Columns: Index No, Student Name, Class, Term, Date, Status, Note</p>
            <button onClick={() => doExport("attendance")} disabled={!!exporting} className="btn-primary w-full">
              {exporting === "attendance" ? (
                ...
                <>
                  ...
                  Export Attendance {filters.format.toUpperCase()}
                </>
              )}
            </button>
          </div>
        ...
        <div className="card p-6 flex flex-col gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="font-bold text-slate-900">Scores and Grades Report</h3>
              <p className="text-sm text-slate-500 mt-0.5">Export class scores, exam scores, totals, grades, and remarks.</p>
            </div>
          </div>
          <div className="mt-auto">
            <p className="text-xs text-slate-400 mb-3">Columns: Index No, Student Name, Subject, Class Score, Exam Score, Total, Grade</p>
            <button onClick={() => doExport("scores")} disabled={!!exporting} className="btn-gold w-full">
              {exporting === "scores" ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin" />
                  Exporting...
                ...
                <>
                  ...
                  Export Scores {filters.format.toUpperCase()}
                </>
              )}
            </button>
          </div>
      ...
      <div className="mt-6 p-4 rounded-xl bg-slate-50 border border-slate-100">
        <p className="text-xs text-slate-500">
          <strong>Note:</strong> Exports include only data from classes and subjects you are assigned to. Filters are optional - leave blank to export all your available data.
        </p>
      </div>
    </div>
  );
